...
function NewsFeedGui:update(t, dt)
	--log("[Blackwave RSS Feed] UPDATING")
	...
	--log("[Blackwave RSS Feed] UPDATED")
end
...
function NewsFeedGui:news_result(success, body)
	...
	if success then
		...
		local titles = {}
		local links = {}
		--log("[Blackwave RSS Feed] parsing a source")
		if string.match(body, "</link>") then
			--log("[Blackwave RSS Feed] </link>")
		...
		--log("[Blackwave RSS Feed] choosing (" .. #titles .. " titles, " .. #links .. " links)")
		...
		if self.urlNumber == 1 then
			...
			for i = 1, self.MAX_NEWS do
				if nums[i] ~= false and titles[i] ~= nil then
					...
					--log("[Blackwave RSS Feed] " .. links[i])
					--counter = counter + 1
				end
			end
		else
		...
		if self.urlNumber == #self.URLS then
			...
			if self.MIX then
			--log("[Blackwave RSS Feed] mixing starts")
				...
				--log("[Blackwave RSS Feed] mixing ends")
			end

			self._news = {i = 0}
			self._next = true
			--log("[Blackwave RSS Feed] announcement starts")
			self._panel:child("title_announcement"):set_visible(#self._titles > 0)
		end
		self.urlNumber = self.urlNumber + 1
	end
	self.busy = false
	--log("[Blackwave RSS Feed] parse func ended")
end
...
function NewsFeedGui:_get_text_block(s, sp, ep, max_results, itemOnly, itemText)
	...
	local function f(s, sp, ep, max_results)
		...
		--log("[Blackwave RSS Feed] insert result e1, s2 " .. e1 .. ", " .. s2)
	...
	while len > i and max_results > #result do
		--log("[Blackwave RSS Feed] get_text_block i=" .. i)
		if itemOnly then
			...
			--log("[Blackwave RSS Feed] item_s " .. item_s)
