...
if not carry_stacker_setup then
	...
	PlayerManager._carry_stacker_dropping = false
	...
	function PlayerManager:drop_carry( ... )
		self._carry_stacker_dropping = true
		ofuncs.managers_player_drop_carry(self, ... )
		self._carry_stacker_dropping = false
		-- skip own stack management if Carry Stacker Reloaded is active
		if not BLT_CarryStacker then
	...
	function PlayerManager:set_carry( ... )
		-- skip own stack push if Carry Stacker Reloaded is active (it manages stacking)
		if not BLT_CarryStacker and not self._carry_stacker_dropping and self:is_carrying() and self:get_my_carry_data() then
			tab_insert(self.carry_stack, self:get_my_carry_data())
		end
