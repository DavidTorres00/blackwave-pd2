-- Blackwave Reborn Trainer - Main Configuration File.
-- To turn options ON write true after '=' , to turn options OFF write false.

return {
-- Blackwave Trainer Options
 -- Keybinds
 ...
 -- Updates & Annoucements
	...
	announcements = false,
	announcements_interval = 60,

 -- HUD Texts
	HUD = true,											-- Set to false to disable ALL Blackwave HUD elements
	HUD_VersionText = true,							-- Displays current version in main menu
	HUD_MovingText = false,							-- Displays moving text in main menu and in game. (disabled - no text configured)
	RSSFeed = false,									-- RSS Feed (disabled - no feed configured)

 -- Logging Option
	LogErrorsToFile = true,							-- Write Blackwave errors to errlog.log
...
-- General Options
 -- Language & Config Options
	Language = 'spanish',							-- Current language. Available languages: English, German, Portuguese, Turkish, Russian, Italian, Spanish, Schinese, Tchinese. Set to false to automatically choose language.
 ...
 -- Anticheat related Options
	...
	Hide_Pirate_Perfection = false,				-- Hides Blackwave from others.
...
-- Tweaks - For Advanced Users only!			-- Toggle here the Tweaks you want to enable.
 ...
 -- Performance Options
	FSSMaxThroughput = 200,						-- Full Speed Swarm AI task throughput cap. Default FSS value is 600 (causes OOM crashes with fire/bags). Lower = more stable. Set to false to leave FSS at its own default.
	ForceGCOnGameStart = true,					-- Force Lua garbage collection when entering a heist. Frees leftover memory from menus.
