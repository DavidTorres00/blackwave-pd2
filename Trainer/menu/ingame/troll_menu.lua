...
-- Per-peer god mode state (globals survive menu reloads)
_bw_peer_god            = _bw_peer_god            or {}
_bw_peer_god_hooks_done = _bw_peer_god_hooks_done or false
...
local function toggle_peer_god_mode( id )
    local lname = "peer_god_" .. tostring(id)
    if _bw_peer_god[id] then
        StopLoopIdent(lname)
        _bw_peer_god[id] = nil
        m_log_error("peer_god_mode", "peer " .. tostring(id) .. " OFF")
    else
        _bw_peer_god[id] = true
        -- Loop: auto-release from custody when unit not alive (~1s interval)
        local tick = 0
        RunNewLoopIdent(lname, function()
            tick = tick + 1
            if tick % 60 ~= 0 then return end
            local peer = M_net_session:peer(id)
            if peer and not alive(peer:unit()) then
                m_log_error("peer_god_mode", "custody detected - releasing peer " .. tostring(id))
                IngameWaitingForRespawnState.request_player_spawn(id)
            end
        end)
        m_log_error("peer_god_mode", "peer " .. tostring(id) .. " ON")
    end
end
...
local MAX_FIRE_ENEMIES = 15  -- cap: 32-bit engine crashes with too many simultaneous fire DOTs

local set_cops_on_fire = function()
	local player_unit = GetPlayerUnit()
	if not alive( player_unit ) then
		m_log_error("set_cops_on_fire", "player not alive")
		return
	end
	if not M_fire then
		m_log_error("set_cops_on_fire", "managers.fire nil")
		return
	end
	...
	local dot_data = {
		...
		dot_length             = 4,  -- reduced from 10: fewer simultaneous particle/damage ticks
	...
	for u_key, u_data in pairs( M_enemy:all_enemies() ) do
		if count >= MAX_FIRE_ENEMIES then break end  -- stop after cap to avoid engine OOM
		if alive( u_data.unit ) then
			local ok, err = pcall(function()
				M_fire:add_doted_enemy({
					unit          = u_data.unit,
			...
			if ok then
				...
				failed = failed + 1
				m_log_error("set_cops_on_fire", "pcall fail: " .. tostring(err))
			end
		end
	end
	m_log_error("set_cops_on_fire", "fired=" .. count .. " failed=" .. failed)
	collectgarbage("collect")  -- reclaim memory from fire DOT object allocation
end
...
-- Instant revive hook for protected peers (runs once per session)
if not _bw_peer_god_hooks_done then
    _bw_peer_god_hooks_done = true
    local _revive = ReviveInteractionExt.interact
    backuper:add_clbk('ReviveInteractionExt.set_active',
        function( r, self, active )
            if not active then return end
            local downed_unit = self and self._unit
            if not downed_unit then return end
            for _, peer in pairs(M_net_session._peers) do
                if peer:unit() == downed_unit and _bw_peer_god[peer:id()] then
                    local ply = GetPlayerUnit()
                    if alive(ply) then
                        m_log_error("peer_god_mode", "revive: peer " .. tostring(peer:id()))
                        _revive(self, ply)
                    end
                    break
                end
...
interaction_with_id_menu = function( id, name )
	...
	local data = { 
		...
		{ text = tr['troll_peer_god_mode'], type = "toggle", toggle = function() return _bw_peer_god[id] end, callback = toggle_peer_god_mode, data = id, host_only = true, switch_back = true },
...
interaction_with_team = function()
	...
	open_menu( { title = tr['troll_interact_team'], button_list = data, back = interaction_with_other } )
end
