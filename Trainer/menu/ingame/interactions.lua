...
local function interactbytweak(...)
	...
	for key,unit in pairs(M_interaction._interactive_units) do
		if alive(unit) then  -- guard: unit may already be freed in C++
			local interaction = unit.interaction
			interaction = interaction and interaction( unit )
			if interaction and tweaks[interaction.tweak_data] then
				insert(interactives, { unit = unit, interaction = interaction })
			end
		end
	end
	for _,data in pairs(interactives) do
		if alive(data.unit) then  -- double-check: unit may be freed between collect and interact
			data.interaction:interact(player)
		end
	end
	if ( unload ) then
...
---------------------------------------------------------------------------------------------------
local function grab_gage_packages()
	local count = 0
	for _, unit in pairs(M_interaction._interactive_units) do
		local interaction = unit.interaction
		interaction = interaction and interaction(unit)
		if interaction and interaction.tweak_data == "gage_assignment" then
			count = count + 1
		end
	end
	interactbytweak("gage_assignment")
	local name = M_network:session():local_peer():name()
	show_hint(string.format(tr.gage_all_grabbed, name, count))
end
...
open_menu = function()
	...
	local contents = {
		...
		{ text = tr.grab_packages, callback = grab_gage_packages },
