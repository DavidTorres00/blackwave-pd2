--Primary scripts for Blackwave Reborn Trainer.
--Purpose: initiate core scripts and requires

-- BeardLib compatibility: preload MenuHelperPlus before init() triggers MenuManagerInitialize.
-- init() fires MenuManagerInitialize early; if BeardLib hasn't loaded Menu.lua yet,
-- MenuHelperPlus is nil and BeardLib crashes. This fix is self-contained — no BeardLib edits needed.
if BeardLib and not MenuHelperPlus then
...
local ME_CREATOR = 'Sulong'
local ME_VERSION = '1.0.0'
local ME_EDITION = 'Blackwave'
local BLT_VERSION = (BLT and BLT.Mods and BLT.Mods:GetModByName("SuperBLT") and BLT.Mods:GetModByName("SuperBLT").version) or 'SuperBLT'
...
if GameSetup then m_log_error("BW_PROBE", "init.lua: TOP reached (game state)") end
...
--Recursively lists files in some directory. Uses BLT native filesystem (no shell spawn).
--Returns table, containing short filenames (without extension).
function rlist_files( path, ext )
	...
	local function scan( dir )
		local files = file.GetFiles( dir )
		if files then
			for _, name in ipairs( files ) do
				if name:sub( -#suffix ) == suffix then
					result[#result + 1] = name:sub( 1, -#suffix - 1 )
				end
			end
		end
		local dirs = file.GetDirectories( dir )
		if dirs then
	...
	scan( 'mods/Blackwave/' .. path )
	if #result > 0 then
		return result
	else
		m_log_error("rlist_files()", "Failed to retrieve files in", path)
	end
end
...
backuper:hijack('TipsTweakData.get_a_tip', function(o, s)
	return math.random() > 0.8 and {image = "general_loot", index = 1, total = 1, title = "Blackwave Reborn Trainer!", text = "Ride the wave. Own the heist.\nv1.0.0 | SuperBLT\nBy Sulong"} or o(s)
end)
...
if GameSetup then --If GameSetup exsists, this means game being setted up.
	m_log_error("BW_PROBE", "init.lua: GameSetup - before auto_ingame")
	ppr_require('Trainer/Setup/auto_ingame')
	m_log_error("BW_PROBE", "init.lua: GameSetup - after auto_ingame")
end